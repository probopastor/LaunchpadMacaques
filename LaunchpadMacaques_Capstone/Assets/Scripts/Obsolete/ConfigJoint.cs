...
[Header("Push Settings")]
    ...
    void StartGrapple(GrappleType grappleType)
    {
        ...
        if (grappleType == GrappleType.Push && Physics.Raycast(m_Camera.position, m_Camera.forward, out hit, maxPushDistance, ~LayerMask.GetMask("CantPush"))
            && CanPush())
        {
            isGrappling = true;
            Instantiate(pushParticle, hit.point, Quaternion.LookRotation((m_Camera.position - hit.point).normalized));
            lr.positionCount = 0;
        ...
        else if (grappleType == GrappleType.Pull && Physics.Raycast(m_Camera.position, m_Camera.forward, out hit, maxPullDistance, whatIsGrappleable) && !pushPull.IsGrabbing())
        {
            ...
            if (!Physics.Raycast(transform.position, dir, distance, whatIsNotGrappleable))
            {
               ...
               // m_audio.m_beam.Play();
                ...
                {
                    pinwheel.TriggerRotation(hitObjectClone.transform, m_Camera.forward);
                }
            }
        }
    ...
    IEnumerator UpdateGrapplePosition(GrappleType grappleType)
    {
        ...
        while (isGrappling)
        {
                ...
                && CanPush())
            {
                ...
                grapplePoint = hit.point + -(m_Camera.forward * maxPushDistance);
